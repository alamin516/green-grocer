...
const ProductImages = ({ productData, setProductData }) => {
  const handleImageChange = useCallback(
    (index, field, value) => {
      const updatedImages = productData.map((image, i) =>
        i === index ? { ...image, [field]: value } : image
      );
      setProductData(updatedImages);
    },
    [setProductData, productData]
  );

  const handleAddImage = useCallback(() => {
    setProductData((prev) => [...prev, { url: "", alt: "" }]);
  }, [setProductData]);
        ...
        productData.map((image, index) => (
          <div key={index} className="flex items-center gap-4 mb-4">
            ...
            <input
              ...
              className="w-1/3 file:mr-4 file:py-2 file:px-4 file:border-0 
